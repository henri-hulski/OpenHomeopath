...
/**
 * help/de/expresstool.php
 *
 * The German manual for the Expresstool.
 *
...
$head_title = "Expresstool :: Hilfe :: OpenHomeopath";
$meta_description = "Handbuch für OpenHomeopath, Expresstool";
if (empty($_GET['popup'])) {
	$skin = $session->skin;
...
<ol>
  ...
  Wenn es die Hauptrubrik noch nicht gibt, kannst du sie gegebenenfalls in die <a href="../../datadmin.php?function=show_insert_form&amp;table_name=main_rubrics">Datenbank einzufügen</a>.</li>
  <li>Gib im Textfeld die <strong>Symptom-Mittel-Datensätze</strong> folgendermaßen ein:
  <ul>
    ...
    <li>Erst wird das <strong>Symptom</strong> inklusiv Elternrubriken angegeben. Die einzelnen Unterrubriken werden mit einem '<strong>></strong>' voneinander getrennt.</li>
    <li>Um nicht alle Elternrubriken immer neu eingeben zu müssen kannst du eine <strong>Kurzform</strong> verwenden:<br>
    '<strong>></strong>' am Anfang einer Symptoms hängt das Symptom an die vorhergehende Elternrubrik an.<br>
    Mit '<strong>>></strong>' am Anfang springt es eine Elternrubrik zurück, mit '<strong>>>></strong>' 2 Elternrubrik usw..</li>
...
?>
  ...
  <li>Wenn eine angegebene <strong>Mittel-Abkürzung</strong> in der Datenbank <strong>nicht gefunden</strong> wurde, wird ein Fehler ausgegeben und der entspechende Datensatz erscheint <strong>zur Korrektur</strong> im Textfeld.<br>
  Überprüfe, ob kein <strong>Rechtschreibfehler</strong> vorliegt, ansonsten überprüfe mit Hilfe der <a href='../../datadmin.php?function=show_search_form&amp;table_name=remedies'>Datenpflege-Suchfunktion</a>, ob in der Mittel-Tabelle eine <strong>andere Abkürzung</strong> verwendet wird. In diesem Fall kannst du die alternative Abkürzung in der <a href='../../datadmin.php?function=show_insert_form&amp;table_name=rem_alias'>Alias-Tabelle hinzufügen</a>.<br>
  Die Mittel-Datenbank enthält mit über <strong>3600 Einträgen</strong> so gut wie alle homöopathischen Mittel. Wenn du wirklich ein neues Mittel hast, kannst du das Mittel in die <a href='../../datadmin.php?function=show_insert_form&amp;table_name=remedies'>Mittel-Tabelle einfügen</a>.<br>
  Bitte das Mittel gegebenenfalls im Textfeld <strong>korrigieren</strong> und das Formular <strong>nochmal abschicken</strong>!</li>
  <li>Wenn die angegebene <strong>Wertigkeit, Status oder Künzli-Punkt</strong> nicht mit der Angabe  der entsprechenden Symptom-Mittel-Beziehung <strong>in der Datenbank übereinstimmt</strong> und der Datenbankeintrag von dir stammt, wird er mit dem angegebenen Wert aktualisiert. Ansonsten bleibt die Original-Eintrag erhalten. Um eine Änderung rückgängig zu machen, kannst du entweder den Datensatz mit der korrekten Angabe nochmal eingeben oder du korrigierst den Datensatz in der <a href="../../datadmin.php?table_name=sym_rem">Datenpflege</a>.</li>
  ...
  <ul>
    ...
    <li><strong>Sprache:</strong> Das Kürzel der Sprache, das in der Sprachen-Tabelle in der Datenbank eingetragen ist (z.B. '<strong>de</strong>' für deutsch, '<strong>en</strong>' für englisch, etc.). Wenn es die Sprache in der Sprachen-Tabelle noch nicht gibt, muss sie erst in die <a href="../../datadmin.php?function=show_insert_form&amp;table_name=languages">Datenbank eingetragen werden</a>.</li>
    ...
    <li><strong>Art der Quelle:</strong> Folgende Angaben sind möglich (schreib beides, deutsch und englisch):
    <ul>
    ...
    Wenn keine Angabe erfolgt wird <strong>"Repertorium / Repertory"</strong> in die Datenbank eingetragen.</li>
  </ul></li>
</ol>
<?php
if (empty($_GET['popup'])) {
