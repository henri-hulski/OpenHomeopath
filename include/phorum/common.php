...
if ( empty( $GLOBALS["PHORUM_ALT_DBCONFIG"] ) || $GLOBALS["PHORUM_ALT_DBCONFIG"]==$_REQUEST["PHORUM_ALT_DBCONFIG"] || !defined("PHORUM_WRAPPER") ) {
    ...
    if (! include_once( "include/db/config.php" )) {
        ...
        if (!file_exists("include/db/config.php")) { ?>
            ...
            <a href="../../doc/INSTALL">doc/INSTALL</a> for installation instructions. <?php
        } else {
...
if(!phorum_db_check_connection()){
    if(isset($PHORUM["DBCONFIG"]["down_page"])){
        ...
        echo "Konnte nicht mit der Datenbank verbinden. &Uuml;berpr&uuml;fe die Datenbankkonfiguration in \"include/classes/db/config_db.php\". Wenn die Konfiguration stimmt, &uuml;berpr&uuml;fe ob der Datenbankserver l&auml;uft.";
        exit();
    }
}
